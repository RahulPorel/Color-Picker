...
const generateNewColorBtn = document.querySelector(".generate-new-color");
...
const generatePalette = () => {
  ...
  for (let i = 0; i < maxPaletteBoxes; i++) {
    ...
    color.innerHTML = ` <div class="rectangle-box" style="background: ${randomHex}"></div>
    <span class="hex-value">${randomHex} </span>`;
    color.addEventListener("click", () =>
      copyColorToClipboard(color, randomHex)
    );
...
const copyColorToClipboard = (elem, hexVal) => {
  const colorEl = elem.querySelector(".hex-value");
  navigator.clipboard.writeText(hexVal).then(() => {
    colorEl.innerHTML = "Copied";
    setTimeout(() => (colorEl.innerHTML = hexVal), 870);
  });
};

generateNewColorBtn.addEventListener("click", generatePalette);
